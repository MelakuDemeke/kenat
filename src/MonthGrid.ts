...
export interface DayHolidayEntry {
    key: string;
    name?: string;
    description?: string;
    tags: string[];
...
export class MonthGrid {
    ...
    _mergeDays(rawDays: any[], holidaysList: Holiday[], saintsMap: Record<number, DayHolidayEntry[]>): (MonthGridDay | null)[] {
        ...
        const labels = (daysOfWeek as Record<string, string[]>)[this.weekdayLang] || daysOfWeek.amharic;
        const monthLabels = (monthNames as Record<string, string[]>)[this.weekdayLang] || monthNames.amharic;
        const holidayMap: Record<string, DayHolidayEntry[]> = {};
        ...
        return (Array(offset).fill(null) as (MonthGridDay | null)[]).concat(mapped);
    }
    ...
    _getLocalizedMonthName(): string {
        return ((monthNames as Record<string, string[]>)[this.weekdayLang] || monthNames.amharic)[this.month - 1];
    }
